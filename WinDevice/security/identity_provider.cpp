...
gm_status Ed25519Identity::generate(std::unique_ptr<Ed25519Identity> &identity) {
    ...
    if (!certificate || X509_set_version(certificate.get(), 2L) != 1 ||
        ...
        X509_time_adj_ex(X509_getm_notAfter(certificate.get()), kCertificateValidityDays, 0L, &now) == nullptr ||
    ...
    if (subject == nullptr ||
        X509_NAME_add_entry_by_NID(subject, NID_commonName, MBSTRING_ASC, common_name, -1, -1, 0) != 1 ||
