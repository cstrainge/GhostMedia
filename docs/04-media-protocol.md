...
wraps, resets, persists, or reuses a value in that epoch. The Windows media sender
initiates rekey before 2^32 packets, 30 minutes, or sequence exhaustion through
`stream.rekey` in document 03. Keep one
...
UDP port supplied over authenticated control. After the Apple output server accepts
`stream.open`, Windows media sender enters `probing` and sends a `PATH_CHALLENGE` in
the Windows-to-Apple direction. Its plaintext is exactly one fresh 96-bit CSPRNG
value. The Apple media receiver returns the same 12 bytes in `PATH_RESPONSE` using
its Apple-to-Windows path key and a new sequence. It never echoes ciphertext, header,
...
deadline. A matching authenticated response consumes its value and authorizes that
tuple for the Windows sender, stream, and epoch for 30 seconds. Unknown, expired,
duplicate, or unmatched responses are dropped. Deadline failure leaves the stream
unstarted or stops it. Receiving a challenge never starts audio: the Apple receiver
responds only for its authenticated candidate tuple, and Windows sends `stream.start`
only after it validates the response. While a stream is sending, Windows refreshes
its authorized path no later than 25 seconds after the prior successful response; a
refresh failure stops the stream before its 30-second authorization expires. Rekey requires a new challenge;
...
negotiated profile. Its `media_timestamp` belongs to the Windows media sender and is
the source-frame timeline after conversion to that profile's network sample rate.
PCM is interleaved signed 16-bit little-endian samples, exactly
...
frames, and timestamps align by 480. The first timestamp may be any u64 chosen by
Windows; whole-packet gaps are permitted after sender queue drops. Reject nonaligned timestamps,
...
After authenticated profile validation, the Apple media receiver inserts by Windows
media timestamp. Keep the first authenticated packet for a timestamp; drop packets
over 120 ms late or 250 ms early. It tracks and reports rendered timestamps but never
generates or renumbers the Windows source timeline.
...
edges, duplicate/reordered packets, Windows sender epoch transition with Apple
receiver overlap, expired and altered path responses, and every ingress budget.
Fixtures contain no production identity, key,
